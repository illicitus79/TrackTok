...
class DashboardApp {
    ...
    async fetchData() {
        try {
            const response = await fetch(`/api/v1/dashboards/project/${this.projectId}`);
            if (response.ok) {
    ...
    renderStats() {
        ...
        // Update card status
        const remainingCard = document.querySelector('[data-card-status="remaining"]');
        if (remainingCard) {
    ...
    updateStat(key, value) {
        ...
        if (element) {
            if (typeof value === 'number') {
                if (key.includes('budget') || key.includes('spend') || key.includes('rate')) {
                    ...
                    tooltip: {
                        ...
                        callbacks: {
                            label: function(context) {
                                ...
                                return `${label}: ${window.TrackTok.formatCurrency(value)} (${percentage}%)`;
                            }
                        }
                    }
                }
    ...
    renderForecastChart() {
        ...
        if (this.data.project.forecast) {
            this.updateStat('projected_total', this.data.project.forecast.projected_total);
